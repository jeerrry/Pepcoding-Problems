...
//			System.out.println("--------------------------");
    ...
    public static boolean isSafe(char[][] ans, int row, int col, char ch, int[] top, int[] left, int[] right, int[] bottom) {
        if ((ans[row][col] != 'X') || (row - 1 >= 0 && ans[row - 1][col] == ch) || (row + 1 < ans.length && ans[row + 1][col] == ch) || (col - 1 >= 0 && ans[row][col - 1] == ch) || (col + 1 < ans[0].length && ans[row][col + 1] == ch)) {
            return false;
        }

        int rowCount = countInRow(ans, ch, row);
        int colCount = countInCol(ans, ch, col);
    ...
    public static boolean isValid(char[][] ans, int[] top, int[] left, int[] right, int[] bottom) {
        ...
        for (int i = 0; i < top.length; i++) {
            if (top[i] != -1 && countInCol(ans, '+', i) != top[i]) {
                return false;
            }
        }
        ...
        for (int i = 0; i < bottom.length; i++) {
            if (bottom[i] != -1 && countInCol(ans, '-', i) != bottom[i]) {
                return false;
            }
        }
    ...
    public static int countInRow(char[][] ans, char target, int row) {
        int count = 0;
        for (char c : ans[row]) {
            if (c == target) count++;
        }
    ...
    public static int countInCol(char[][] ans, char target, int col) {
        int count = 0;
        for (char[] ar : ans) {
            char c = ar[col];
            if (c == target) count++;
